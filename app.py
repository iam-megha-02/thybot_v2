...
def document_chat_page():
    ...
    st.markdown("Upload a document (`PDF`, `DOCX`, `TXT`) to begin a chat session about its contents.")
    ...
    if uploaded_file:
        with st.spinner("Processing document..."):
            ...
            st.session_state["doc_chunks"] = load_and_split_document(uploaded_file)
            if st.session_state["doc_chunks"]:
                st.session_state["doc_faiss_index"] = embed_chunks(st.session_state["doc_chunks"])
    ...
    if "doc_faiss_index" in st.session_state:
        ...
        if prompt := st.chat_input("Ask a question about the document..."):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    ...
                    system_message_content = (f"You are ThyBot, an expert AI assistant. Your task is to answer questions based ONLY on the provided document context. The user's thyroid status is '{thyroid_type}'. Your response style should be {st.session_state.response_mode}.\n\nCONTEXT:\n---\n{context}")
        ...
        if st.button("Start New Document Chat"):
            del st.session_state["doc_chunks"]
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigation", ["Patient Profile", "Document Chat", "Meal Analysis"])
